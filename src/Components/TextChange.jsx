...
const TextChange = () => {
  ...
  useEffect(() => {
    const intervalId = setInterval(() => {
      ...
      if (isForward) {
        ...
        if (endValue < 1) {
          setIsForward(true);
          setIndex((prev) => (prev + 1) % texts.length); // Cycle to next
        }
      }
    }, 50); // Typing speed
  ...
  return (
    <div className="transition ease duration-300 text-xl md:text-6xl font-bold leading-normal tracking-tighter">
      {currenText}
